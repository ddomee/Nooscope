...
def generate_vault_layout(vault: str | None = None) -> dict:
    """Scan the vault and write a layout reference document to 05 - Resources/Vault Layout.md.

    Creates a structured markdown document describing the vault's folder structure,
    note counts, top frontmatter tags, most-linked notes (by [[wikilink]] count), and
    key locations. Safe to re-run — overwrites the existing document.
    """
    ...
    # Collect [[wikilink]] targets across the whole vault (strips aliases/#headings)
    ...
    for note, count in link_counts.most_common(20):
    ...
    return {
        ...
        "most_linked": dict(link_counts.most_common(10)),
    }
...
_VAULT_LAYOUT_PATH = "05 - Resources/Vault Layout.md"
_VAULT_LAYOUT_MISSING = (
    "The vault layout reference document (05 - Resources/Vault Layout.md) is missing. "
...
def _load_vault_layout(config) -> str:
    """Read 05 - Resources/Vault Layout.md from the primary vault.
